...
| ffmpeg | **Detect on system** (`ffmpeg`/`ffprobe` in PATH or configured path). First-run setup screen offers guided install + `scripts/fetch-ffmpeg.mjs` (downloads a static build to `~/Library/Application Support/OpenLoom/bin`). We do NOT bundle GPL binaries in the repo | Keeps the repo small and its licensing self-contained. |
...
| License | **AGPL-3.0-or-later** (OSI open source; plain MIT up to v0.1.1, MIT + Commons Clause for v0.1.2-v0.1.3 - see LICENSE and docs/DECISIONS.md) | Real open source for adoption, with reciprocity: a hosted fork must publish its changes. |
...
- **S7. Publish to YouTube (unlisted), Data API upload, shipped v2 (2026-07-24, supersedes the guided-manual v1):** one click uploads the recording's `video.mp4` to the user's own channel via the resumable `videos.insert` endpoint and returns the watch link - no Finder reveal, no drag-drop, no paste-back. Auth is the installed-app OAuth loopback (RFC 8252) + PKCE with bring-your-own Google Cloud "Desktop app" credentials entered in Settings › YouTube (nothing baked into the binary; consent opens in the system browser because Google blocks embedded-window logins; the refresh token is stored encrypted via the existing settings-secret path). The S6 forced-private constraint is unchanged: an un-audited API project locks uploads to private regardless of the requested privacy, so the app requests `unlisted`, reads back the applied privacy onto `VideoMeta.youtubePrivacy`, and shows a one-click "Set to Unlisted" (opens the Studio edit page) whenever it is `private`. True hands-off unlisted = pass the project's YouTube API compliance audit (`docs/YOUTUBE-PUBLISH.md`). Pure, unit-tested builders (PKCE, auth-URL, loopback-callback, videos.insert metadata) in `youtube-core.ts`; OAuth in `youtube-oauth.ts`; upload in `youtube.ts`. IPC: `youtubeStatus`, `youtubeConnect`, `youtubeDisconnect`, `youtubePublish(videoId)`, `youtubeOpenStudioEdit(videoId)` (additive to section 5; see docs/DECISIONS.md).
